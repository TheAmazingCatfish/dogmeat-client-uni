...
exports.main = async (event, context) => {
    ...
    let payload = {};
    let noCheckActions = [
        'register', 'checkToken', 'refreshToken', 'encryptPwd', 'login', 'loginByWeixin', 'sendSmsCode',
        'setVerifyCode', 'loginBySms', 'loginByEmail', 'code2SessionWeixin', 'code2SessionAlipay'
    ]
    if (noCheckActions.indexOf(event.action) === -1) {
        if (!event.uniIdToken) {
            return {
                code: 403,
                msg: '缺少token'
    ...
    switch (event.action) {
        case 'register': {
            const { username, password, needPermission } = params;
            res = await uniID.register({ username, password, needPermission });
            break;
        }
        case 'login': {
            const { username, password, needPermission } = params;
        ...
        case 'bindMobile': {
            const { uid, mobile, code } = params;
            res = await uniID.bindMobile({ uid, mobile, code });
            break;
        }
        case 'unbindMobile': {
            const { uid, mobile, code } = params;
            res = await uniID.unbindMobile({ uid, mobile, code });
            break;
        }
        case 'bindEmail': {
            const { uid, email, code } = params;
            res = await uniID.bindEmail({ uid, email, code });
            break;
        }
        case 'unbindEmail': {
            const { uid, email, code } = params;
            res = await uniID.unbindEmail({ uid, email, code });
            break;
        }
        case 'getUserInfo': {
            const { uid, field } = params;
            res = await uniID.getUserInfo({ uid, field });
            break;
        }
        case 'code2SessionWeixin': {
            const { code } = params;
            res = await uniID.code2SessionWeixin({ code });
            break;
        }
        case 'loginByWeixin': {
            const { code } = params;
        ...
        case 'bindWeixin': {
            ...
            res = await uniID.bindWeixin({ uid, code });
            break;
        }
        case 'unbindWeixin':
        ...
        case 'code2SessionAlipay': {
            const { code } = params;
            res = await uniID.code2SessionAlipay({ code });
            break;
        }
        case 'loginByAlipay': {
            const { code } = params;
            res = await uniID.loginByAlipay({ code });
            break;
        }
        case 'bindAlipay': {
            const { uid, code } = params;
            res = await uniID.bindAlipay({ uid, code });
            break;
        }
        case 'unbindAlipay': {
        ...
        //     const { uid } = params;
        //     const oldToken = event.uniIdToken;
        //     if (!oldToken) {
        //         return {
        //             code: '',
        //             message: '非法请求'
        //         };
        //     }
        //     const { total } = await users.where({
        //         _id: uid,
        //         token: oldToken
        //     }).count();
        //     if (total == 1) {
        //         const { token, tokenExpired } = await uniID.createToken({ uid });
        //         const { updated } = await users.doc(uid).update({
        //             token: db.command.push(token)
        //         });
        //         if (updated == 1) {
        //             res = {
        //                 code: 0,
        ...
        case 'setUserInviteCode': {
            ...
            res = await uniID.setUserInviteCode({ uid });
            break;
        }
        case 'acceptInvite': {
            const { uid, inviteCode } = params;
        ...
        case 'addRole': {
            const { roleID, roleName, comment, permission } = params;
        ...
        case 'getRoleList': {
            const { offset, limit, needTotal } = params;
        ...
        case 'addPermission': {
            const { permissionID, permissionName, comment } = params;
        ...
        case 'getRoleByUid': {
            const { uid } = params;
            res = await uniID.getRoleByUid({ uid });
            break;
        }
        case 'bindRole': {
            const {
